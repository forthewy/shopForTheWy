...
<div class="d-flex justify-content-center">
	<div class="item-create-box w-75">
		<h1 class="pb-3">상품 수정</h1>
			<%-- 상품 명 --%>
			<div class="d-flex align-items-center">
				<label for="name" class="mb-3">상품명</label>
				<input type="text" id="name" name="name" class="ml-2 form-control col-4 mb-3" value="${item.name}">
			</div>
			<%-- 상품 분류 --%>
			...
			<div class="mb-3">
				...
				<div class="item-content-box bg-white">
					<textarea id="content" name="editordata">${item.content}</textarea>
				</div>
			</div>
			<%-- 상품 가격 --%>
			<div class="d-flex align-items-center">
				<label for="price" class="mb-3" >가격</label>
				<input type="text" id="price" name="price" value="${item.price}" class="form-control col-4 ml-2 mb-3">
			</div>
			<%-- 상품 수량 --%>
			<div class="d-flex align-items-center">
				<label for="number" class="mb-3">수량</label>
				<input type="text" id="number" name="number" value="${item.number}" class="form-control col-4 ml-2 mb-3">
			</div>
			<%-- 배송비 --%>
			<div class="d-flex align-items-center">
				<label for="deliveryPrice" class="mb-3">배송비</label>
				<input type="text" id="deliveryPrice" name="deliveryPrice" value="${item.deliveryPrice}" class="form-control col-4 ml-2 mb-3">
			</div>
			<%-- 대표 이미지 --%>
			<div>
			...
			<div class="d-flex justify-content-center">
				<button type="button" id="itemUpdateBtn" class="btn btn-primary col-4 mb-4" data-item-id="${item.id}">수정</button>
			</div>
	</div>
</div>
...
$(document).ready(function() {
	...
	$('#itemUpdateBtn').on('click', function(e) {
		...
		$.ajax({
			...
			, error:function(e) {
				alert("상품 수정에 실패했습니다. 관리자에게 문의주세요");
			}
		}); // ajax 끝
	}); // 수정 끝
});
